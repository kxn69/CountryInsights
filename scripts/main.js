...
// get graph-wrapper and graph-title class
...
function searchThroughCountries() {
  ...
  const filteredCountries = countries_data.filter((country) => {
    ...
    return (
      country.name.match(regex) || capital.match(regex) || languages.match(regex)
    );
  })
...
searchBar.addEventListener('input', searchThroughCountries);
nameButton.addEventListener('click', handleNameButton);
capitalButton.addEventListener('click', handleCapitalButton);
populationButton.addEventListener('click', handlePopulationButton)
populationGraphButton.addEventListener('click', showPopulationContent);
languagesButton.addEventListener('click', showLanguagesContent);
